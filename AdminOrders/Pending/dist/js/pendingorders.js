...
let fetchPendingOrders = async () => {
    
    var orders = await UnApprovedOrder();
    console.log(orders.status);
    if(orders.status == false)
    {
      myBody.innerHTML = "";
      myBody.innerHTML = `<p>${orders.message}</p>`
    }
    else
    {
        orders.data.forEach(purchase => {
     ...
     for(let i = 0; i < 1; i++)
       {
        orders.data.forEach(p => {
               ...
               let tot = 0;
               p.cart.productCustomerCarts.forEach(pp => {
                   ...
                   <td>${pp.productPrice * pp.productQuantity}</td>
                   </tr>`
                   tot += pp.totalPrice;
...
{
  fetch(`https://localhost:44331/api/Order/ApproveOrder/${orderId}`, {
    method: "POST",
  })
  .then((res) => 
  {
...
async function SearchDate()
{
    ...
    let orders = await fetch(`https://localhost:44331/api/Purchase/ViewPurchaseByDate/${collectedDate.value}`)
    // .then((res) => 
    // {
    //     return res.json();
    // })
