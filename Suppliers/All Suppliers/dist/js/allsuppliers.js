...
{
    let suppliers = await fetch(`https://localhost:44331/api/Supplier/AllSuppliers`)
    let mySuppliers = suppliers.json();
    return mySuppliers;
}

let fetchAllSuppliers = async () => {
    
    var suppliers = await AllSuppliers();
    console.log(suppliers);
    suppliers.data.forEach(sale => {
            ...
            <div class="card-body">
                <div style="display: flex;">
                    <div>
                        ...
                        <h4 style="display: inline;"><input value="" id="cuNames" readonly style="border: none;"/></h4>
                    </div>
     ...
     let myTab = document.querySelectorAll('.tab');
      ...
      for(let i = 0; i < 1; i++)
        {
            suppliers.data.forEach(p => {
                let count = 0;
                console.log(p);
                if(p.purchases.length < 1)
                {
                    myTab[i].innerHTML = `<h3 style="padding-left: 20px;">No purchases registered under this supplier yet</h3>`
                }
               
                p.purchases.forEach(pp => {
                   console.log(pp);
                    count++;
                    table[i].innerHTML +=
                    ` <tr>
                    <td>${count}</td>
                    <td>${pp.referenceNo}</td>
                    <td>${pp.dateCreated}</td>
                    <td>${pp.totalAmount}</td>
