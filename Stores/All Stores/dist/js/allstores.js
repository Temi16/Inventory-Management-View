...
{
    let suppliers = await fetch(`https://localhost:44331/api/Store/AllStores`)
      ...
      for(let i = 0; i < 1; i++)
        {
            suppliers.data.forEach(p => {
                ...
                {
                    myTab[i].innerHTML = `<h3 style="padding-left: 20px;">No sections registered in this store</h3>`
                }
               
                p.sections.forEach(pp => {
                   console.log(pp);
                    count++;
                    table[i].innerHTML +=
                    ` <tr>
                    <td>${count}</td>
                    <td>${pp.sectionName}</td>
                    <td>${pp.sectionDescription}</td>
                    </tr>`
